...
bounded recovery retries. The run **stops on purpose**, because a wrong angle
...
| Refuses targets outside 0 to 25 mm | `ALLOW_OVERTRAVEL` disabled | it is `True` by default; the code jogs past the limit and suppresses the expected exception. **Never clamp the target** |
...
`--renull-mode fast-renull`, which is slower but re-optimises everywhere.
...
| Check the null background | the null-point row measures the contaminating phasor directly, and `s9_analyser_independent_fraction` quantifies it |
...
| Current normal, no optical response | electrically fine, no (or cancelled) electro-optic response, which could be genuine or a badly multi-domain region |
...
| Cause | Fix |
| --- | --- |
| $V_c$ outside ±40 V | classified `partial_loop_unresolved`. 40 V is a **hard** ceiling that you cannot raise |
...
| Wrong operating point | the loop uses the chip-sweep peak conditions, so check the map is good at that pixel |
...
| Lever | Cost |
| --- | --- |
| Reduce the dwell | changes the loop shape, and is only valid if you change it for the *whole* campaign |
...
| Do **not** enable the adaptive fine grid to save time | it is *slower*, because it adds a reconnaissance loop before the real one |
...
redo, untick the box, or delete that pixel's folder, keeping a copy first.
...
| An older CSV has no X/Y columns | historical files stored magnitude and phase only | the analysis reconstructs X/Y automatically, and it is fully retroactive |
...
> **Warning** Close these before starting a run, because they hold the
> instrument sessions and the worker will then fail to connect.

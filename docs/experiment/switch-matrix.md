...
- **Signal plane.** The **SMU4201** (DC, 0…±40 V, 1 mA compliance) and
  **TGF3162 CH1** (AC, 1–9 Vpp at 30 kHz) are summed by the **bias tee** onto a
  single common drive bus. The closed relay connects that bus to one electrode
...
> bit-bangs with `digitalWrite()` — but the pin choice leaves that upgrade
...
4. **Drop `LAT` low** so the outputs hold their previous, already-latched state
   ...
   configuration throughout the shift — there is no glitch window.
...
| Send | Meaning | Reply |
| --- | --- | --- |
| `1` … `100` | **logical pixel** — the firmware looks it up in its own `PIXEL_TO_ELECTRICAL_SWITCH[]` array | `-> Logical Pin <n> -> Electrical Switch <s>` |
...
clone Nanos this setup uses, the old bootloader then sits for roughly 1.5–2 s
...
prove a dead board — but `banner_seen()` records the fact, and the interactive
